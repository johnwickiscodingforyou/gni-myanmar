...
export default function AboutPage() {
  const [views, setViews] = useState<{total:number;today:number;countries:Record<string,number>} | null>(null)

  useEffect(() => {
    // Track this visit
    fetch('/api/track', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ page: '/about' }) }).catch(() => {})
    // Fetch view counts
    fetch('/api/article-briefs?limit=1')
      .then(r => r.json())
      .catch(() => {})
  }, [])
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 mb-6">
          ...
          <p className="text-gray-300 text-sm leading-relaxed">
            This Myanmar version presents GNI intelligence reports in Myanmar language, making global geopolitical analysis accessible to Myanmar readers at zero cost.
          </p>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-5 mb-6">
          ...
          <p className="text-xs text-gray-400 leading-relaxed">GNI Myanmar reads intelligence data from GNI_Autonomous via secure API. GNI_Autonomous runs 4 autonomous pipelines daily producing geopolitical intelligence reports, MAD multi-agent debate verdicts, and market analysis — all at $0.00/month cost.</p>
          <a href="https://gni-autonomous.vercel.app" target="_blank" className="inline-block mt-3 text-xs text-blue-400 border border-blue-800 rounded px-3 py-1">View GNI_Autonomous</a>
        </div>

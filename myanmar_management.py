...
def run_mad(supa, run_date, run_ts, report_data):
    ...
    log(f"  Existing rounds: R1={'done' if existing.get('mad_r1_bull') else 'pending'} | "
    ...
    if existing.get("mad_r1_bull"):
        ...
        # Save R1 immediately -- website updates!
        if mad_r1:
            try:
                ...
                log("  OK: R1 saved to debate_summaries -- website updated!")
    ...
    # ===== FINAL SAVE -- assemble full mad_mm =====
    log("\n-- Final: Assemble full MAD summary --")
    try:
        final = _fetch_existing(supa, run_date)
        mad_mm_parts = []
